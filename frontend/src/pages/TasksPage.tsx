...
import { Check, Circle, Plus, Search, X } from "lucide-react";
...
export default function TasksPage() {
  ...
  const remoteTaskItems = useMemo<TaskItem[]>(() => {
    ...
    return (taskQuery.data?.items ?? []).map((task) => {
      ...
      return {
        ...
        status: task.status === "completed" ? "done" : dueDate < today ? "overdue" : dueDate === today ? "today" : "upcoming",
        assignee: {
          id: task.assignee_id,
          name,
  ...
  const visible = useMemo(() => {
    ...
    return sourceTasks.filter((task) => {
      ...
      const matchesSearch = !needle || `${task.title} ${task.entity} ${task.assignee.name}`.toLocaleLowerCase("ru").includes(needle);
      return matchesFilter && matchesSearch;
    });
  }, [filter, search, sourceTasks]);
