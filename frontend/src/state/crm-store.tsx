...
export function CrmProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
    ...
    ]).then(async ([pipelines, loadedSources, loadedUsers]) => {
      if (!active) return;
      ...
      const mappedPipeline = mappedPipelines.find((item) => item.id === selectedPipeline.id) ?? mappedPipelines[0];
      ...
      const mappedDeals = dealPages.flatMap((page) => page.items)
        .filter((deal) => deal.pipeline_id === selectedPipeline.id)
        .map((deal) => mapRemoteDeal(
          deal,
          mappedPipeline.stages,
          loadedUsers,
          loadedSources,
      ...
      setNextCursorByStage(Object.fromEntries(
        selectedPipeline.stages.map((stage, index) => [stage.id, dealPages[index]?.next_cursor ?? null]),
      ));
      setDeals(mappedDeals);
      setError(null);
    }).catch((reason: unknown) => {
      if (!active) return;
      console.error("Pulse CRM data bootstrap failed", reason);
      setError("Не удалось загрузить данные CRM");
    }).finally(() => {
      if (active) {
  ...
  const loadMoreDeals = useCallback(async (stageId: string) => {
    ...
    try {
      ...
      const mapped = page.items
        ...
        .map((deal) => mapRemoteDeal(deal, pipeline.stages, users, sources));
      setDeals((current) => {
      ...
      items.map((deal) => {
        ...
        return {
          ...
          tasks: deal.tasks.map((task) => {
            ...
            return { ...task, completed, version: task.version + 1 };
          }),
        };
      }),
    );
