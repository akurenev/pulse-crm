...
На телефоне этапы Kanban прокручиваются горизонтально, а табличный вид
перестраивается в компактные карточки. Перенос между этапами выполняется
select-полем карточки, а не drag-and-drop. Вкладки карточки содержат детали,
задачи, переписку и историю. Пользовательские поля сделки можно свернуть;
контакт, компания и теги редактируются кнопкой-карандашом. Круглая кнопка **+**
...
`GET /api/v1/companies`. Истории переходов хранятся отдельно для двух списков;
поиск и создание записи возвращают соответствующий список на первую страницу.
