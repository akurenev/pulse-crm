...
- переименовать любой этап — `PATCH /api/v1/stages/{stage_id}`;
...
Удаление защищено от потери данных. Можно удалить только пустой открытый этап:
системные этапы `won`/`lost` защищены, а в воронке должен остаться хотя бы один
этап `open`. Воронку можно удалить, только если она не последняя, в ней нет
сделок и на неё не ссылаются каналы, формы, webhook-и или правила оповещений.
