...
контактом или компанией. Редактирование и завершение выполняются
`PATCH /api/v1/tasks/{id}` с текущим `expected_version`; PATCH также позволяет
заменить или снять связь со сделкой, контактом и компанией. Удаление задачи —
`DELETE /api/v1/tasks/{id}?expected_version=N`. Удаление физическое, поэтому UI
