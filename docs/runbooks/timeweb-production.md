...
- S3 bucket приватный; созданы префиксы `attachments/`, `imports/`, резервный
  `exports/` и `backups/` (штатного массового экспорта в MVP нет); для пилота
  допустим минимальный стандартный тариф с
