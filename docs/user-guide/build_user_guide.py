...
def add_cover(doc: Document) -> None:
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("Версия MVP 0.1  |  29 августа 2026")
...
def build_document() -> None:
    ...
    doc.core_properties.modified = datetime(2026, 8, 29, tzinfo=timezone.utc)
    ...
    add_screen_page(
        ...
        "Управление воронкой и этапами в настройках Pulse CRM",
        [
            "«Новая воронка» создаёт рабочие и финальные этапы; название воронки должно быть уникальным.",
            "Карандаш у воронки меняет название; «+» добавляет рабочий этап с названием и цветом перед финальными (до 50 этапов).",
            "Карандаш этапа меняет название и цвет; корзина удаляет только неиспользуемый рабочий этап.",
            "Финальные этапы можно переименовать, но нельзя удалить; в воронке должен остаться хотя бы один рабочий этап.",
            "Корзина удаляет не последнюю активную воронку без сделок, истории переходов и связанных настроек.",
        ...
        callout=(
            "Важно",
            "Входящий лид не теряется из-за незаполненных полей. При отказе удаления устраните указанную зависимость. Если данные изменил другой администратор, повторите действие после обновления.",
            "warning",
        ),
        max_width=6.0,
    )
    ...
    add_table(
        ...
        [
            ...
            ["Воронка или этап не удаляется", "Объект используется сделками, историей или настройками. Перенесите данные и перенастройте указанную зависимость."],
