...
![Центр уведомлений Pulse CRM с разными событиями](user-guide/screenshots/01-notifications.jpg)

*Рисунок 2. Центр уведомлений с описаниями разных событий*
...
2. Найдите сделку по названию, источнику, тегу, пользовательскому полю,
   данным связанного контакта или компании. Поиск охватывает все этапы
   выбранной воронки, включая успешные и закрытые сделки.
...
2. Заполните **Название**, **Организация**, **Контакт** и **Сумма, ₽**.
   Начните вводить организацию или контакт: если CRM найдёт подходящую запись,
   выберите её из списка — связь сохранится в карточке. Новые названия можно
   просто ввести вручную.
...
- Кнопка **Список** показывает каждую сделку отдельной карточкой: название,
  организацию, этап, сумму, источник, срок и ответственного. На большом экране данные
...
- Поиск выполняется на сервере сразу по всем этапам выбранной воронки, включая
  **Успешно** и **Закрыто**, даже если финальные колонки ещё не были загружены.
  Учитываются название, источник, теги и пользовательские поля сделки, имя,
  Email и телефон связанного контакта, а также название, Email и телефон
  связанной компании.
- Если найденных сделок больше одной страницы, нажмите **Показать ещё
  результаты**. Следующая порция продолжает тот же поиск и не заменяет уже
  показанные карточки.
...
- На телефоне удаление и закрытие карточки находятся на отдельной верхней
  панели, по разным краям экрана и с подписями. Это отделяет опасное действие
  от остальных элементов карточки.
- На широком экране карточка открывается увеличенной панелью: этап воронки,
  удаление и закрытие находятся рядом и не перекрывают друг друга. Нажмите
  карандаш рядом с названием и суммой в заголовке, чтобы изменить их и сохранить
  одним действием. Для длинного названия используется многострочное поле, а в
  карточке оно переносится без обрезания. Сумма может быть равна нулю.
...
- Если контакт или компания уже связаны со сделкой, нажмите их название, чтобы
  открыть соответствующую карточку в разделе **Клиенты**. Кнопка-карандаш
  по-прежнему служит для изменения самой связи.
...
- Используйте поиск по имени, тегу, телефону или Email. Для компаний поиск
  дополнительно учитывает ИНН, сайт и значения пользовательских полей. Результаты
  запрашиваются на сервере после короткой паузы ввода и остаются на экране до
  получения нового ответа, поэтому найденная компания не должна исчезать из-за
  локальной повторной фильтрации. Импортированные теги контактов показаны под
  именем, а теги компаний — в их списке и карточке.
...
- Для компании переключите вид на **Компании**, затем укажите название, ИНН
  (10 или 12 цифр), Email, телефон и сайт.
...
Карточка контакта показывает связь с компанией, все связанные сделки, теги,
накопленную выручку, число покупок, плановую дату и историю.
...
4. Если контакт связан с компанией, нажмите название в поле **Компания**, чтобы
   открыть её карточку. Переход доступен ролям, которым разрешён раздел
   компаний.
5. Блок **Сделки** показывает все доступные сделки контакта — открытые,
   успешные и закрытые. Нажмите строку сделки, чтобы открыть её карточку, даже
   если она находится в другой воронке, на другой странице или в финальном
   этапе.
6. Показатели **Покупок** и **Покупок на сумму** считаются только по выигранным
   сделкам, хотя в блоке **Сделки** доступны все этапы.
7. В поле **Добавить заметку** зафиксируйте договорённость и нажмите
   **Добавить**.
...
Карточка компании объединяет реквизиты организации, связанные контакты и
историю изменений.

![Карточка компании с ИНН, связанным контактом и кнопкой редактирования](user-guide/screenshots/08-company-card.jpg)

*Рисунок 10а. Карточка компании с ИНН и связанными контактами*

1. Переключитесь на **Компании** и нажмите строку организации.
2. ИНН показан среди основных реквизитов. Нажмите **Редактировать**, чтобы
   изменить название, ИНН, основной Email, телефон, сайт или теги. Допустимы
   ИНН из 10 или 12 цифр; после сохранения карточка и список обновятся.
3. В блоке **Контакты компании** показаны связанные люди с их телефоном и
   Email. Нажмите контакт, чтобы открыть его карточку без повторного поиска.
4. В поле **Добавить заметку** сохраните важную информацию о компании.
5. Кнопка-корзина удаляет компанию после отдельного подтверждения; связанные
   контакты и сделки сохраняются, но больше не ссылаются на неё.
...
Правило связывает событие, получателя, канал, задержку и текст шаблона. В
стандартном шаблоне используются подстановки `{event_title}` и
`{event_summary}`, поэтому в уведомлении видно, что именно произошло: например,
новый лид, входящее сообщение или просроченная задача. Не удаляйте
`{event_summary}`, если хотите сохранить такое описание в push.
...
компаний, сделок, задач, полей и заметок. ИНН компании переносится из обычного
поля **ИНН** либо из реквизита ИНН поля **Юр. лицо**. Обычные примечания amoCRM,
включая многострочный текст и коды, появляются в блоке **Заметки** карточки
сделки. Теги контактов, компаний и сделок переносятся вместе с соответствующей
записью. Воронки переносятся вместе с этапами; одинаковые системные этапы в
разных воронках обрабатываются раздельно.
...
> [!TIP]
> После обновления Pulse CRM повторный импорт безопасно заполняет ИНН у ранее
> перенесённых компаний, теги у ранее перенесённых сделок и ранее невидимые
> примечания. Значения обновляются из amoCRM, а сами записи не дублируются.
...
push не копируются. Заголовок и текст push показывают название и краткое
описание события, например новый лид, входящее сообщение или просроченную
задачу. Нажатие на уведомление о сделке или задаче открывает именно
