...
- В строке **Теги** нажмите **Изменить**, перечислите теги через запятую и
  сохраните. Пустые значения и повторы удаляются автоматически; на Kanban
  показываются первые два тега и счётчик остальных.
